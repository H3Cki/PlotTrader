...
func TestMin_At(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			plots: []geometry.Plot{
				&geometry.Schedule{Until: time.Unix(10, 0), Plot: &geometry.Line{B: 1}},
				&geometry.Line{B: 1.5},
			},
			t:    time.Unix(9, 0),
			want: 1,
...
func TestMax_At(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			plots: []geometry.Plot{
				&geometry.Schedule{Until: time.Unix(10, 0), Plot: &geometry.Line{B: 2}},
				&geometry.Line{B: 1},
			},
			t:    time.Unix(9, 0),
			want: 2,
...
func (v *neverValid) At(time.Time) (float64, error) {
	return v.returns, geometry.ErrOutOfRange
}

func (v *neverValid) InRange(time.Time) bool {
	return false
}
